export default `section#top.Header
...
section#basics.Section
    ...
    p Secondly, there is <b>places</b>, which contains an array of data objects for each of the local authorities in the dataset (it filters out regions, countries etc.). This can be used to find eg. the highest or lowest place for a particular metric, eg. §<b>#{'#{[...places].sort((a,b) => a.column_name - b.column_name)[0].areanm}'}</b>§ would give the name of the place with the highest value.
...
section#demo-files.Section
    h2 Simple article demo
    p If you want to skip straight to the action, there's a demo CSV dataset and PUG template file so you can see how it works. #[button(onclick="ddjDemo()" class="btn-inline") Click here] to load it.

section#logic-nlg.Section
    h2 NLG/logic demo
    p PUG and RosaeNLG also allow for the possibility of introducing logic and natural language generation (NLG) into your templates. #[button(onclick="nlgDemo()" class="btn-inline") Click here] to see an example, using the same CSV file as above.

section#save-output.Section
